...
RUN_LOG="$OUTPUT_DIR/architect-$DATE.md"
...
try:
    ...
    for i, f in enumerate(d.get('findings', []), 1):
        ...
        print()
        print(f'**Motivation:** {f[\"motivation\"]}')
        print()
        files = f.get('files', [])
        if files:
            print('**Files:**')
        ...
        print(f.get('proposed_approach', ''))
        print()
        seq = f.get('sequencing_notes', '')
        if seq:
            print(f'**Sequencing:** {seq}')
            print()
        print('---')
        print()
except Exception as e:
...
echo "  📝 Report saved: $RUN_LOG" | tee -a "$RUN_LOG"
...
try:
    ...
    lines.append(f.get('proposed_approach', ''))
    lines.append('')
    seq = f.get('sequencing_notes', '')
    if seq:
...
" 2>/dev/null || echo "See architect report: $RUN_LOG")
