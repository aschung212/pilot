...
### 2026-08-30 — Discovery lost its web research twice over: a model bump killed grounding, and the "Claude self-researches" fallback never existed

- **Symptom.** The 22:00 discovery run logged `⚠️ Gemini research FAILED — API error 429: You exceeded your current quota` and continued. It did not crash; it filed 4 issues (LIFT-1274/1275/1276/1277) with **zero external research**.
- **Root cause 1 — the model bump.** A 2026-08-29 model audit moved `AI_RESEARCH_MODEL` from `gemini-2.5-flash` to `gemini-3.6-flash` to reset the free tier's per-model daily bucket, which triage (~5 calls/run) had drained and discovery (1 call/run) shares. But **grounded calls draw on a separate, smaller free bucket than ungrounded ones**. Verified live on 2026-08-30: `2.5-flash` answers both grounded and ungrounded; `3.6-flash` answers ungrounded only, 429ing every grounded call. So the bump fixed triage and silently cost discovery the one thing Gemini is in the pipeline *for*. Reverted to `2.5-flash`, with the reasoning written into `project.env` so the next audit does not repeat it.
- **Root cause 2 — the fallback was a lie.** `DISCOVER_ALLOWED_TOOLS` was `Read,Glob,Grep,Bash(gh:*),git…` — **no `WebSearch`, no `WebFetch`**. The Phase 1 failure path has always claimed "Claude will self-research (degraded quality)"; with no web tools, Claude could not research at all and silently degraded to pure codebase introspection. This has been true for every Gemini failure in the repo's life (4 since the 2026-07-19 REST migration). The 08-30 run recorded it in its own search log: `SEARCH:WebSearch … — BLOCKED (tool not permitted this run)`. Added `WebSearch,WebFetch` to the allowlist. This is the same class as the cleanup.sh "detector that reports instead of acting" defect — a message naming a mechanism nobody implemented.
- **Also found, not yet fixed.** (a) `adapters/ai-research.sh` parses only `candidates[0].content.parts` and **discards `groundingMetadata`** — Gemini returns 16 grounding chunks per call that are thrown away, which is why its research arrives uncited (0 URLs across 13 of 14 post-migration runs). (b) `triage.sh:344` sets `TRIAGE_MODEL="gemini-2.5-flash"` as a *reporting label* but never passes `--model`, so triage actually runs on whatever `AI_RESEARCH_MODEL` says — the label has been wrong since the 08-29 bump.
- **Measured, 2026-08-30 (n=1 per arm, `competitors` focus, discovery's exact Phase 1 prompt).** Gemini 2.5 Flash grounded: 50s, 17.4 KB, **0 URLs, 0 prices, 0 ratings, 0 version numbers** — despite the prompt demanding them. Claude Sonnet 5 + WebSearch: 137s, 8.7 KB, **24 URLs across 14 domains**, 6 star-ratings, dated events (Hevy Trainer 2026-02-18, Strava sync 2026-06), and one genuinely exploitable competitor complaint (StrongLifts locking legacy lifetime buyers behind $12/mo). Sonnet also *declined to fabricate* Reddit permalinks it could not retrieve and said so — the opposite of the hallucinated-`liftracker.app` failure mode.
- **New responsibility for Aaron.** None day-to-day. Two standing rules: **never bump `AI_RESEARCH_MODEL` to dodge a rate limit without checking the new model answers GROUNDED calls** (`ai-research.sh prompt "…"` with no `--no-grounding`), and if the grounded bucket runs dry again the fix is to cut Gemini call *volume* — triage is ~5 calls/run against discovery's 1 — not to bump the model.

### 2026-08-30 — Discovery research moved to Claude Sonnet 5, behind a backend toggle with automatic fallback

- **What.** `adapters/ai-research.sh` now has **two backends behind one interface** — `claude` (headless `claude -p` with `WebSearch`/`WebFetch`) and `gemini` (the existing Gemini API Flash + Search grounding). `scripts/discover.sh` picks via `DISCOVER_RESEARCH_BACKEND` (`claude` | `gemini` | `claude-only` | `gemini-only`), and **the backend you don't pick becomes the automatic fallback**, so one vendor's quota wall or 503 no longer costs discovery the web. Default is `claude`. Triage is untouched: it keeps calling the adapter's default backend (`AI_RESEARCH_BACKEND=gemini`), ungrounded and free.
- **Why — measured, not assumed.** Bake-off on discovery's own Phase 1 prompt, `competitors` focus, one run per arm:

  | | Gemini 2.5 Flash | **Sonnet 5** | Opus 5 (max) |
  |---|---|---|---|
  | Wall clock | 50s | **137s** | 1187s |
  | URLs / domains | **0 / 0** | 24 / 14 | 26 / 4 |
  | Live links | — | 13/14 | 13/14 |
  | Ratings / prices / versions | 0 / 0 / 0 | 6 / 2 / 1 | 0 / 7 / 10 |
  | Cost equiv. | $0 | ~$1.35 | ~$7.93 |

  Gemini returned **zero URLs on a prompt that explicitly demands URLs**, and that is the norm — 0 URLs in 13 of 14 runs since the REST migration. Sonnet's App Store figures were re-checked against Apple's iTunes Lookup API and matched **exactly** (Strong v6.5.0 / 4.86 / 108,505; StrongLifts v4.3.0 / 76,628; Fitbod v8.31.0 / 282,708). Both Claude arms flagged what they could *not* source instead of inventing it — Sonnet explicitly refused to fabricate Reddit permalinks. Given the hallucinated-`liftracker.app` incident, that behaviour matters more than raw volume.
- **Sonnet, not Opus.** Opus was marginally more precise but took **20 minutes and ~6× the tokens**, largely by fanning out to `Task` subagents. Phase 1 runs ahead of triage and the builder, so the claude backend **disallows `Task`** to keep latency and cost bounded, and is wall-clock capped by `AI_RESEARCH_CLAUDE_TIMEOUT` (420s). A timeout now reports *as a timeout* rather than as "unparseable output", which would have sent whoever reads the 3am alert after the wrong bug.
- **Second reason to move.** Triage (~5 calls/run) and discovery (1 call/run) shared one per-model daily Gemini bucket — the exact contention that caused the 08-29 model bump and, through it, the outage above. With discovery on Claude, Gemini's whole budget belongs to triage.
...
### 2026-08-31 — Lift: the log-set annotations cost 112px unconditionally, and three of nine RPE values were unreachable (#1289, PR #1290)

- **What Aaron reported.** In the log-set sheet the "Went for one more" toggle and the RPE selector stack vertically, eating screen space — and once RPE is expanded there is no way to collapse it again.
- **The height was worse than it looked.** Both rows already sat at the 44pt floor, so the sheet spent **112px on them unconditionally** — identical whether or not either annotation was used — while each pill occupied well under half its row. Together they need ~241px of the 333px available, so they now share one row: **60px at rest, 108px with the scale open**, i.e. still under what the old layout cost for doing nothing.
- **Root cause of the collapse bug.** Expanding the scale **replaced** the `RPE` button with the chip strip, so the only route back was re-tapping the chip that was already selected — a real path, but one nothing on screen suggested. The pill now stays put holding the value and flipping its chevron: the control that opens the scale is the control that closes it.
- **Two further defects found while fixing, neither reported.** (a) Nine 44pt chips need 428px and the row gives 350, so **RPE 9, 9.5 and 10 sat off-screen** behind `overflow-x: auto` with no scroll indicator — a third of the scale was reachable only by a gesture nothing hinted at. The scale is now five whole points plus a half-step modifier: six chips fit outright, every rating is at most two taps away. (b) Tapping the RPE button ran `selectedRPE = 7`, so **opening the picker and dismissing it saved a 7 the user never chose** — a silent data-write on any set where the row was merely opened.
- **And one I introduced and caught.** The ½ chip reads quieter than the value chips, but `.wtRPEHalfChip` and `.wtRPEChipActive` are both single-class, so an unscoped muted colour won on source order and painted `--text-muted` onto the accent fill — measured **3.31:1, under the 4.5:1 AA floor**, on a chip that looks selected. Scoped with `:not()`; 12.24:1 after, pinned by a test.
- **No migration.** Base and half are DERIVED from the single stored value rather than held beside it, and the stored value set is unchanged (6–10 in half steps, exactly what `parseGuards` accepts). Existing history is untouched.
- **Design first, then code.** Four directions were mocked against the real `index.css` tokens before choosing; the two runners-up (an inline stepper with no disclosure at all, and a full-width segmented bar) are kept on a second page of the design canvas with why they were not taken.
- **Verified live, not just in tests.** Driven at a 375pt viewport: one 44px row (156 + 85 of 333), six 50×44 chips on a single line, the half riding across a point change (8.5 → tap 9 → 9.5) and clamping at 10 since 10.5 is not a rating. 17 new tests (11 behavioural, 6 structural). Local suite 3781/3781; CI green on all gates.
- **Found in passing — a latent red that CI cannot see.** `composableSSRSafety.test.ts` deletes the global `localStorage` and never restores it, so the shared `afterEach` in `setup.ts:65` throws and **a full local `npm test` is red on master** (reproduced on an untouched `origin/master` worktree at ce869f2). CI stays green only because `ci.yml` shards the suite across a 3-way matrix and the file happens to land apart from its collision partners — that is luck, not isolation, and the shard boundary moves whenever files are added or renamed. Filed separately, not folded into this PR.
- **New responsibility for Aaron.** None. PR #1290 is open and green, awaiting his merge.

### 2026-09-13 — Lift: fixed and merged the three remaining open PRs (#1409, #1400, #1404)

- **What.** All three open Lift PRs are squash-merged and master is green: **#1409** (LIFT-1354) fixes the deploy denylist Vercel had rejected, **#1400** (LIFT-1398) client-side `bar_weight` repair, **#1404** (LIFT-1401) restores the missing `updated_at` triggers on prod.
- **#1409 needed a real fix, not just a merge.** Its commit shipped a 465-character `ignoreCommand` inline in `vercel.json`; Vercel's schema caps that field at 256 and every deployment on the branch had been erroring on schema validation — invisible to CI, since the PR's own GitHub Actions run had never triggered at all (a separate, apparently-transient miss; pushing a new commit re-triggered it). Confirmed via Vercel's deployment API (`readyState: ERROR`, exact message). Moved the pathspec list to `scripts/vercel-ignore-build.sh`; `vercel.json` now just runs `bash scripts/vercel-ignore-build.sh` (35 chars), and `deployVerification.test.ts` gained a regression pin on the field's length so it can't silently regrow past the cap.
- **#1400 and #1404 both conflicted with same-day master merges, and both were ordering conflicts, not competing intent.** #1400's only conflict was a migration COMMENT that LIFT-1397 (merged first, same day) had already superseded with a more detailed rewrite covering the identical correction — resolved by taking master's file verbatim. #1404's conflict was two independently-appended test `describe` blocks in `architecturalInvariants.test.ts` (LIFT-1392, already on master, and this PR's own LIFT-1401 block) landing at the same tail position; git's common-suffix matching had silently reassigned LIFT-1392's own closing braces to the very end of the file, which would have left one `describe` block syntactically unclosed if resolved by pattern-matching the diff alone. Caught by running the file's own test suite (85/85) before continuing the rebase, not by reading the diff.
- **Discovered a race in the deploy-verification pipeline while merging back-to-back.** Merging three PRs ~90s apart made `smoke-test-production` false-fail for the middle commit: Vercel deployed it successfully (confirmed `READY` via the deployments API), but by the time that job's own dependencies (`build-and-test` + `e2e` + `migrate-db`, ~6 min) finished, the next push had already superseded it as the production alias — so the job polled for a commit that could never come back, and `notify-failure` posted a false 🔴 to `#lift-automation`. Filed as [#1414](https://github.com/aschung212/Lift/issues/1414) rather than fixed inline: a real gap (the job assumes at most one push lands per ~6-minute CI window), but rare in normal one-PR-at-a-time review and out of scope for this session.
- **New responsibility for Aaron.** The `🔴 Post-merge CI failed on master` message in `#lift-automation` around 2026-09-13 05:15 UTC, for PR #1400's commit, is a false alarm — both that deploy and the one right after it are verified live on production. #1414 tracks the real fix (accept a descendant commit as verified, not just an exact SHA match).
- **Tests.** All four gates (lint / typecheck / vitest / build) green locally on each PR before merge; full CI (incl. e2e + Lighthouse) green on all three before merging. Final master state: 4400 tests passing.

### 2026-09-15 — Lift: Apple Health bodyweight write-sync on the native iOS build (#1420, PR #1421)

- **What.** Aaron asked to "upload and sync the bodyweight data I have logged in the app to my Apple Health account." HealthKit has no web API, so this lands on the native Capacitor build only: a new **Settings → Apple Health** group (rendered only when `isNative && platform === 'ios'`) with a *Sync bodyweight* switch. Turning it on requests write-only `weight` access, backfills every existing weigh-in, then writes each new one as it is logged — including entries arriving from another device via the store fetch. Status row: syncing / N waiting / up to date / denied (with the Health › Sharing path) / error (with *Sync now*). The PWA keeps the #1159 CSV export as its path.
- **How.** Plugin `@capgo/capacitor-health` (the only maintained Capacitor HealthKit plugin with a write API; `capacitor-health` is read-only), dynamically imported behind the native gate. Pure `src/lib/healthSync.ts` (lbs→kg, sample instant, device-local write-once ledger bound to the user who enabled it, read-back match against Lift's own samples so a reinstall or second iPhone never duplicates); `useHealthSync` (single-flight + trailing re-run, ledger marked after each save); `setupHealthSync()` in App.vue subscribes the bodyweight store. `scripts/configure-ios.mjs` runs from Capacitor's `capacitor:sync:after` hook and re-applies to the per-machine, uncommitted `ios/` project the HealthKit usage strings, the entitlement + `CODE_SIGN_ENTITLEMENTS`, and the iOS 16 deployment target (#531's former manual step). `@capacitor/ios` added; `npx cap add ios --packagemanager SPM` generated the project without CocoaPods.
- **Deliberate limits.** Write-once: edits and deletions in Lift are NOT propagated — the plugin can neither delete nor update a sample and HealthKit's `HKMetadataKeySyncIdentifier` upsert is out of its reach; the Settings hint says so. Sample instant is local NOON for `endOfDayISO` stamps (the raw `…T23:59Z` files a Tokyo weigh-in under the next day — #746 again), pinned on both sides of UTC.
- **New responsibility for Aaron.** (1) Review + merge [PR #1421](https://github.com/aschung212/Lift/pull/1421) (CI gates it). (2) **Accept the Xcode license first** — every `xcodebuild`/`simctl` call on the Mac (and `/usr/bin/git`, `python3`) currently exits with the license error, which blocked simulator verification this session: `sudo xcodebuild -license accept`. (3) Then verify natively: `npm run cap:build && npm run cap:open:ios`, run on a Simulator, Settings → Apple Health → toggle, check Health › Body Mass. (4) For his own phone: the free Personal Team may refuse the HealthKit entitlement (#216 assumed paid enrollment; the Simulator needs no signing) — if Xcode rejects the profile, that is the trigger to decide on enrollment. Until the native build runs on his iPhone, the CSV export remains the way to get data into his real Health account.
- **Tests.** 4504 passed (229 files); lint/typecheck/build clean; a real `npx cap sync ios` ran the hook ("HealthKit config already applied"); web Settings sheet verified in the browser (group hidden off native, no console errors). New suites: `healthSync.test.ts`, `useHealthSync.test.ts`, `configure-ios.test.mjs` (runs against the real template inside `@capacitor/cli`), plus SettingsSheet / dates / appMeta additions.

### 2026-09-15 — Lift: Apple Health sync verified on the iOS Simulator; a Capacitor proxy bug found and fixed (#1420, PR #1421)

- **What.** Aaron accepted the Xcode license, so the native build was run for real: `npx cap add ios --packagemanager SPM` → `npm run cap:build` → headless `xcodebuild` (69 s, first build) → iPhone 17 Pro / iOS 26.4 Simulator. Settings → Apple Health renders natively; the switch raises the HealthKit sheet (write-only, Weight, our usage copy); Allow flips it to "Up to date · synced Sep 15"; the onboarding sample entries are skipped; logging 171.6 lb on the Weight tab produces `checkAuthorization → readSamples → saveSample` on the bridge and the Health app then shows Weight 172 lb from Lift.
- **Bug found by the run.** The first tap did nothing. `loadPlugin` resolved a promise with the `Health` export, which is a Capacitor `registerPlugin` Proxy whose `get` trap turns EVERY property read into a plugin call — `then` included — so the engine's thenable assimilation fired a native `Health.then()` that rejected ("not implemented on ios", unhandled) while the awaiting `enable()` hung forever. Fixed by handing the proxy back inside a plain object; the composable test now builds its fake through the REAL `registerPlugin` (the old plain-object fake had no `then` and could not see it — which is how 4504 green tests shipped it), a new test races `enable()` against a timeout and reports "hung" on the old loader, and `architecturalInvariants.test.ts` bans any dynamic `@capacitor/`/`@capgo/` import chained with `.then(`. Documented in Lift's CLAUDE.md.
- **How it was diagnosed (worth keeping).** WebView console output never reaches the unified log; `xcrun simctl launch --console-pty <udid> <bundle>` streams Capacitor's `⚡️ To Native` / `[error]` lines (plain `--console` is block-buffered and shows nothing). System permission sheets ignore the simulator tool's plain taps; `touch_path` (real touch-down/up) works. For Simulator builds Xcode embeds the *Simulated* entitlements into the binary, so `codesign -d --entitlements` shows an empty dict even though HealthKit is granted.
- **Pre-existing native-shell defects seen (not fixed, filed as an issue under #534).** The WebView does not fill the bottom safe area (a white band under the tab bar) and the layout stays shifted after the keyboard dismisses; the app-icon picker's native `AppIcon` plugin does not exist yet, so it logs "not implemented" on every launch.
- **New responsibility for Aaron.** Review + merge [PR #1421](https://github.com/aschung212/Lift/pull/1421) (second commit adds the fix; CI gates it). To use it on his own iPhone: same `cap:build` + Xcode run with his signing team; if Xcode refuses the HealthKit entitlement under the free Personal Team, that is the #216 enrollment decision.
- **Tests.** 4506 passed (229 files); lint/typecheck/build clean; the regression test was shown to fail against the old loader.

### 2026-09-15 — Lift: the three native-shell defects from the Simulator run fixed (#1423, PR #1424)

- **What.** (1) `ios.contentInset` was `'automatic'`, so the WebView inset its scroll view for safe areas the page already handles (`viewport-fit=cover` + `env(safe-area-inset-*)`); the layout viewport shrank by ~93pt and the WebView's own background showed as the white band under the tab bar. Now `'never'` (Capacitor's default). (2) `Keyboard.resize` was `'body'`, which rewrote `document.body.style.height` around every keyboard and left the page scrolled and shifted after dismissal. Now `'native'` (Capacitor's default): the WebView itself resizes, so the 100svh shell and the sheets' sticky action bars follow the keyboard like a native screen. (3) The app-icon picker rendered on every native build while its Swift half does not exist yet, logging `"AppIcon" plugin is not implemented on ios` on each launch; it is now gated on `Capacitor.isPluginAvailable('AppIcon')` and reappears the moment the plugin ships (#531).
- **Verified on the Simulator.** Page paints edge to edge with the tab bar floating above the home indicator; Log Weight holds position with the keyboard up and restores exactly on Save (the entry synced to Health as before); Settings → Appearance ends at Units; console shows `resize mode - native` and zero AppIcon errors.
- **Guards.** `capacitorConfigRegression.test.ts` pins both config values with the reasoning (nothing on the web can see either failure); `nativeAppIcon.test.ts` covers the unregistered-plugin branch; `SettingsSheet.test.ts` proves the picker renders only when the plugin is registered. Lift's CLAUDE.md native-readiness section records both rules.
- **New responsibility for Aaron.** Review + merge [PR #1424](https://github.com/aschung212/Lift/pull/1424) (CI gates it). Still open by design: #1422 (edits/deletions propagating to Health) needs a decision — commit the `ios/` project for a custom Swift plugin, or an upstream PR to the capgo plugin.
- **Tests.** 4514 passed (229 files); lint/typecheck/build clean.

### 2026-09-15 — Lift: first physical-iPhone run — Health sync verified on Aaron's phone; the Settings dev tools were shipping in the production native build (#1425, PR #1428)

- **What.** Aaron ran the Capacitor build on his iPhone 17 Pro Max under a free Personal Team — HealthKit signs fine without paid enrollment, so #216's assumption was wrong in the good direction. Two blockers on the way in, neither an app bug: the Xcode project had been generated in a git worktree, which does not inherit the main checkout's gitignored `.env.local`, so the bundle shipped with "Supabase not configured" (copied the two `VITE_SUPABASE_*` lines, rebuilt; [#1427](https://github.com/aschung212/Lift/issues/1427) documents it); and Google OAuth cannot run inside the WKWebView (Google refuses embedded web views, and the redirect target is `capacitor://localhost`), so Aaron gave his Google-created user a password through the GoTrue admin API and signed in with email + password ([#1426](https://github.com/aschung212/Lift/issues/1426) filed for the real fix: system-browser OAuth with a deep-link callback). The keyboard did not appear on the auth screen for the first Xcode-launched instance and did after a plain relaunch — a device quirk, not a defect. Health sync then ran end to end on the phone: write access granted, one read-back query, 140 weigh-ins saved, zero errors; the Health app's 6-month chart matches Lift's.
- **Bug found by the run, fixed and merged.** Settings on the phone rendered the **Dev Tools** group (Reset Onboarding, Seed 12k/80k XP, +5,000 XP, Run Migration, Clear All Data) in a production bundle. The gate was `window.location.hostname` matching localhost/LAN, and the bundled Capacitor app is served from `capacitor://localhost` — true on every native install, App Store build included. Nothing on the web can see it, and happy-dom's hostname is `localhost` too, so every SettingsSheet test mounted WITH the tools and none asserted their absence. The group now lives in `src/views/DevToolsGroup.vue`, lazily imported by SettingsSheet behind `import.meta.env.DEV || VITE_E2E` (the LIFT-1123 shape that keeps "Continue as Dev" out of AuthScreen), so a production build never emits the chunk — absent, not hidden. Native live reload keeps the tools because `DEV` is true there.
- **Guards.** `scripts/check-no-dev-surface.js` (renamed from `check-no-dev-signin.js`; CI runs it after the build) greps `dist/` for both dev surfaces; `prodBundleGuard.test.ts` is generalised over both — host renders nothing inline, gates the lazy import on the build flags, never reads `location.hostname` — and caught two of my own comments quoting a marker while the fix was written; `SettingsSheet.test.ts` flips the build flag with `vi.stubEnv` and asserts the group is absent even at hostname `localhost`; `architecturalInvariants.test.ts` fails any `location.hostname` read outside `supabase.ts`'s Vercel-preview routing. Lift's CLAUDE.md gained the rule.
- **Verified.** 4526 tests green, lint/typecheck clean, the production build carries no dev markers (`check-no-dev-surface` ✅ 2 surfaces / 48 JS files); a fresh Simulator install of the native bundle shows Exercises → Gyms → Weight Goal → Apple Health with no Dev Tools where the phone screenshot had them. Merged as 7c0cf49 after every CI gate (incl. e2e + Lighthouse) passed.
- **New responsibility for Aaron.** (1) His phone still runs the earlier bundle; the worktree's native bundle has been rebuilt from master, so the fix is one Run in Xcode away. (2) A free-team profile expires after 7 days — rerun from Xcode when the app stops launching. (3) Parked decisions, all in issues: #1426 (native Google sign-in), #1427 (README `.env.local` note), and #1422 now records option 3 — an in-repo Capacitor plugin installed as a `file:` dependency, recommended over committing `ios/` or waiting on an upstream PR — to pick up before the App Store submission.

### 2026-09-15 — Lift: App Store path — every unblocked item done as four PRs; the rest is Aaron-only and written down (#216)

- **What.** Aaron decided to enroll in the Apple Developer Program to share Lift with friends, which fires the #216 trigger. Everything code could do without the membership is on four PRs, each green on CI and verified on the Simulator where it renders:
  - [#1429](https://github.com/aschung212/Lift/pull/1429) `ios/` **committed as the App Store build** (closes #531, #536, #538): the Lift icon replaces Capacitor's placeholder, the launch screen is the PWA's Eternal mark at 2732² (`generate-launch-screens.js --native`), `PrivacyInfo.xcprivacy` is wired into the target, iPhone-only, `ITSAppUsesNonExemptEncryption=false`, and `configure-ios.mjs` stamps `MARKETING_VERSION` from package.json and the build number from the commit count on every `cap:build`. `LiftApp/` deleted. Its test now also asserts the committed project carries every transform, so a hand edit in Xcode fails CI.
  - [#1431](https://github.com/aschung212/Lift/pull/1431) **password reset by emailed code** (closes #1430) — the first reset path Lift has ever had. Supabase's PKCE recovery link only completes in the origin that requested it, which the native app can never be, so the auth screen verifies the emailed 6-digit code in place (`verifyOtp` → `updateUser`); the link path still works on the web via a `PasswordResetSheet`. The native auth screen offers email/password + guest only until #1426 + #542 ship together (App Review 4.8).
  - [#1432](https://github.com/aschung212/Lift/pull/1432) **privacy policy + terms at public URLs** (closes #537) from one source (`src/lib/legalCopy.ts`), emitted at build to `/legal/privacy.html` and `/legal/terms.html`; the policy now covers Apple Health, the AI coach (Anthropic), Sentry, in-app deletion (the old text still said to email), every third party. The legal copy tipped the 640 KB JS budget master was within 1 KB of; raised to 672 KB on both PRs.
  - [#1433](https://github.com/aschung212/Lift/pull/1433) **`docs/app-store/`**: the submission path in order, the listing fields, the App Privacy questionnaire row by row (mirrors the manifest and the policy), the TestFlight runbook, the review checklist (#539, #540, #541); its second commit closes #1427 (README: a worktree starts without `.env.local`).
- **Tracker.** #533 closed (done on master), #535 closed (superseded by TestFlight), #534 and #216 carry status comments.
