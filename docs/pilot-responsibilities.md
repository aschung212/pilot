...
- Test suite (bats-core, 287 tests across 23 files): fast tier (280 tests) runs on every commit via pre-commit hook, full tier (287 tests) runs on push via GitHub Actions CI
...
| `~/development/pilot/tests/` | bats-core test suite — 23 test files, 287 tests (fast tier, 280, runs in the pre-commit hook) |
