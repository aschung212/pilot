...
- **Streaks are per workflow.** A green `npm audit` interleaved with red CI runs would otherwise zero the streak — the precise shape of the bug being fixed. Its first live run found a second unknown outage: `Integration Tests`, red for the whole scan window — and, once investigated, red for all 95 of its runs since May (Lift #1283, fixed 2026-08-31).
